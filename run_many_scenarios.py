...
def main_args(num_runs, base_dir, forced_scenario, assistant, rounds, skip_runs: bool, log_dir, out_file, service = "LLM"):
        ...
        _, system, _ = next(scenario for scenario in SCENARIOS if scenario[0] == forced_scenario)
        
        if not skip_runs:
            run_scenario_multiple_times(num_runs, base_dir, forced_scenario, assistant, rounds, system, service)
        ...
        mean_cost, std_cost, mean_time, std_time, mean_action_number, std_action_number = compute_stats_from_logs(log_files)

        append_results(out_file, mean_cost, std_cost, mean_time, std_time, num_runs, forced_scenario, system, log_files_interval, mean_action_number, std_action_number)
        ...
        print(f"Mean suggestion time: {mean_cost:.6f}")
        print(f"Std dev of suggestion time: {std_cost:.6f}")
...
def main():
    ...
    main_args(args.num_runs, base_dir, args.forced_scenario, args.assistant, args.rounds, args.skip_runs, log_dir, out_file, args.service)
...
if __name__ == "__main__":
    ...
    main_args(10, base_dir, 0, "EvidenceKGOptimal", 10, False, log_dir, out_file)
    ...
    main_args(10, base_dir, 6, "EvidenceKGOptimal", 10, False, log_dir, out_file)
    ...
    main_args(10, base_dir, 12, "EvidenceKGOptimal", 10, False, log_dir, out_file)
