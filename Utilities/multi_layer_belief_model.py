...
class MultiLayerBeliefModel:
    ...
    def get_delta(self):
        ...
        pieces_of_evidence = [get_set(e)
                              for e in self.quantitative_evidence.keys()]
        self.delta = {}

        # For every size r of combination of pieces of evidence
        for r in range(0, n+1):
            # For all combinations of size r
            for combination in combinations(pieces_of_evidence, r):

                remaining_evidence = [
                    e for e in pieces_of_evidence if e not in list(combination)]

                value = 1.0

                for evidence in combination:  # For each piece of evidence in the combination
                    # The value is multiplied by the new value
                    value *= self.quantitative_evidence[get_key(evidence)]

                for evidence in remaining_evidence:  # For each piece of evidence not in the combination
                    # The value is multiplied by 1 - the new value
                    value *= 1 - self.quantitative_evidence[get_key(evidence)]

                # We add the delta for this combination
                self.delta[str(list(combination))] = value
    ...
    def get_delta_tau(self):
        ...
        for (combination, value) in delta:  # Browsig entries of delta function
            # Get the delta entry as a set
            argument = ast.literal_eval(combination)

            if not argument:  # If delta entry is the empty set, new entry is S
                entry = self.space
            elif self.allocation_function == minimal_dense_set:
                # Compute allocation function on list
                entry = self.allocation_function(argument)
            else:
                # Compute allocation function on list
                entry = reduce(self.allocation_function, argument)
    ...
    def get_justification_frame(self):
        ...
        if self.justification_frame == 'dempster_shafer':
            ...
            # Remove the empty set from the topology
            justification_set.remove(set())
        elif self.justification_frame == 'strong_denseness':
            justification_set = topology.get_dense_sets()
        else:
            raise ValueError(
                "Invalid value for J (Justification Frame). Must be 'dempster_shafer' or 'strong_denseness'.")
    ...
    def get_normalization_factor(self):
        ...
        # Since delta_tau only contains elements with non-zero value, there may be j that are not in delta_tau
        denominator = sum(delta_tau[get_key(j)]
                          for j in justification_set if get_key(j) in delta_tau)
    ...
    def get_delta_J(self):
        ...
        for justification in justification_set:  # For all justification j
            ...
            # If delta_tau(j) exists (has non-zero value)
            if key in delta_tau:
                self.delta_J[key] = delta_tau[key] / \
