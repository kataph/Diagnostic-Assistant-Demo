...
def async_disk_cache_runner_run(func):
    ...
    async def wrapper(agent, *args, **kwargs):
        ...
        cache_logger.info(
            f"[CACHE MISS] {agent_id} / key={key[:8]}... / raw={raw}")
...
def async_disk_cache_CLI(func):
    ...
    async def wrapper(*args, **kwargs):
        ...
        cache_logger.warning(
            f"CLI CACHING ACTIVATING WITH use_cache {use_cache}...")

        if not use_cache:
            return await func(*args, **kwargs)

        # Build cache key
        raw = (func.__module__, func.__qualname__,
               [repr(x) for x in args[1:]], kwargs)
        key = hashlib.sha256(pickle.dumps(raw)).hexdigest()
