...
class SaboteurHuman(Saboteur):
    ...
    async def sabotage(self, description: SystemDescription) -> Optional[RootCauseDescription]:
        ...
        y_n = ""
        y_n = input(
            "Do you want to record a description of the root cause (for logging purposes)?  y/n\n>").lower()
        while y_n not in ["y", "n"]:
            y_n = input(
                "You have to write either 'y' or 'n', try again\n>").strip().lower()
        if y_n == "n":
            self.logger.info(
                f"The system was saboted producing an unrecorded root cause")
            return None
        ...
        while True:
            symptom_description = input(
                prompt_for_human_collect_symptoms_description)
        ...
        rc = RootCauseDescription(
            root_cause_description_proper=rc_description, symptoms_descriptions=symptom_descriptions)
        self.logger.info(
