    ...
    def __init__(self, description: SystemDescription, configuration: Configuration):
        ...
        self.constrainedoutputdiagnoser_v3 = Agent(
            ...
            instructions=(
                ...
                "  - action_target: Always name one or more specific component(s) names (e.g. 'battery', "
                "'main_bulb', 'relay', 'psu_green_resistor', 'control_input_cable'), NOT a module "
                "name (e.g. NOT 'power supply module', NOT 'control module'). If you target a "
                "module/enclosure instead of a component, the repair will attempt to replace the "
                "entire module (costly and usually wrong). Enclosures may be target of actions "
                "(e.g. move/invert/rotate enclosure/cube), similarly some actions may have multiple "
                "targets (e.g. swap cable1 with cable2)\n"
                "  - action_description: a SHORT, SIMPLE description based on the pattern VERB + "
                "TARGET. Examples: 'Measure voltage at battery terminals', 'Test continuity of "
                "main_bulb', 'Replace psu_green_resistor'. Do NOT write lengthy explanations — "
                ...
                "  Provide suspected_components: a list of actual component/enclosure IDs (not descriptions). "
                "Use only the exact IDs that appear in the system description, e.g. "
                "'battery', 'main_bulb', 'psu_cable_pos', 'cube_ctrl1', 'load_diode'. "
                "Do NOT invent or describe components.\n"
                ...
                "issue). Use ONLY the real component IDs from the system."
            ),
