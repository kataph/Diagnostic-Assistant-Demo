...
SCENARIOS = [
    ...
    RootCauseDescription(
        ...
        symptoms_descriptions = SymptomDescriptions([
            SymptomDescription("The led on the power supply module is on"),
            SymptomDescription("The lamp turns of about every 20 seconds for about half a second. Then it turn on again. This keeps happening."),
        ])
    )),
]
...
class SaboteurFixedScenario(Saboteur):
    ...
    async def sabotage(self, description: SystemDescription) -> Optional[RootCauseDescription]:
        _, system, rc = next(scenario for scenario in SCENARIOS if scenario[0] == self.configuration.FORCED_SCENARIO_ID)
